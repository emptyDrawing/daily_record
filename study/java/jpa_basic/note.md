...
![](assets/2023-04-11-09-24-07.png)

    - 이렇게 관계에 ( fetch = FetchType.LAZY) 를 해주면 지연로딩

    ![](assets/2023-04-11-09-26-35.png)

    - 그리고 팀을 일단 조회만 하면 proxy 강제 초기화 하지 않는 이상? 쿼리는 안나감.

    ![](assets/2023-04-11-09-28-40.png)

- 그런데 Member 랑 Team를 자주 같이 사용하면? ( EAGER )

    - ![](assets/2023-04-11-09-30-43.png)

    - ![](assets/2023-04-11-09-31-09.png)

    - 그래서 진짜가 나옴.

- 그런데 실무에서는 최대한 `지연로딩(Lazy)`
    - 예상치 못한 쿼리가 나올수 있음. ( Join 이 ... 속도가.. )
    - 즉시로딩시 `JPQL` 인 경우 N+1 문제 발생

    ![](assets/2023-04-11-09-38-17.png)

        - 사실 Lazy 를 써도 loop로 Team를 쓰면 하나하나 나가진 하지만
        - 1) FetchJoin 으로 한방에 가져올 수 있고

        ![](assets/2023-04-11-09-47-58.png)

        - 2) @EntityGraph
        - 3) Batch Size 등으로 풀면 됨. ( 이때는 1 + 1 )

    - `@ManyToOne` / `@OneToOne` 기본이 `EAGER` 니까 꼭 변경 ( `@OneToMany`, `@ManyToMany` 는 기본이 `LAZY` )

- 지연로딩 활용은 결국 그냥 필수다.

### 영속성 전이 : CASCADE /  고아객체

- 부모를 저장할때 자식도 저장하고 싶을때?
- 적확한 단어로 부모를 영속하고 싶을대 자식도 같이 영속

    ![](assets/2023-04-11-09-54-17.png)

    ![](assets/2023-04-11-10-08-21.png)

    - 위처럼 하고 싶을때임. persist를 3번하지 않고...

    ![](assets/2023-04-11-10-09-40.png)

- 즉 Persist 를 어떻게 해줄꺼냐 문제임. `연관관계 맵핑` 과는 아무런 관련이 없음.
- 편의성임
  - ALL : persist / remove 둘다
  - PERSIST / REMOVE : 영속 / 삭제

- 그런데 이것도 관리하는 주체가 여러개 있으면 사용하면 안됨.

- 고아객체란 사실 제거가 중요한데 부모엔티티와 관계가 끈어진 자식 엔티티 자동 삭제 같은거.
  - `orphanRemoval = true` 

  ![](assets/2023-04-11-10-29-38.png)

- 그럼 둘다 키면 무슨 의미일가?

![](assets/2023-04-11-10-36-44.png)

    - 자식은 repository 를 따로 만들필요 가 없어진다.
    - AggregateRoot 가 관리할 수 있다.
...
- 값 타입
  - 식별자가 없고 값만 가지고 있어서 추적 불가능, Int, String 처럼 단순 값으로만 취급하면 타입, 객체
  - 이 값타입에 기본값 / 임베디드 / 컬렉션 값 타입이 있음.	

- 기본값이 자바기본타입, 래퍼클래스, String 같은거
- 임베디드랑 컬렉션 값 타입은 따로 JPA에 정의해서 써야함. ( 좌표 같은거 )
- 그래서 값타입은 생명주기를 엔티티에 의존하고, 공유하면 안됨.

- 임베디드 타입
  - 내장타입이라고도 불리는데.. 새로운 값 타입을 직접 정의하는 뜻
  - 주로 기본값 타임을 모아서 만들어 복합값 타입이라고도 함.
  - 그런데 값 타입이라는걸 주의 ( 추적 변경 안되야함. )
  - 예를 들어 회원 엔티티에 근무시간 집주소 같은게 있는데 여기서 임베디드 타입이 `근무시간` `집주소`이 될 수있음.

    ![](assets/2023-04-11-13-28-34.png)

  - 사용법은 `@Embeddable` : 정의하는 곳 `@Embedded` : 사용할 곳

    ![](assets/2023-04-11-13-59-00.png)

    ![](assets/2023-04-11-13-59-30.png)
    - 원래 한쪽만 정의해도 되지만 양방향으로 넣자.
 
  - 임베디드 타입을 사용하기 전후 맵핑하는 테이블은 같지만 좀더 객체지향적으로 활용하기는 쉬워진다.

  - ![](assets/2023-04-11-14-07-26.png)
  - 임베디드 타입이 내부에 엔티티를 가질수도 있음.


  - 그런데 한 엔티티에서 같은 값을 사용하면...?

  ![](assets/2023-04-11-14-09-07.png)

  - 집주소/ 직장주소 같은 경우 -> `@AttributeOverrides()` 를 사용

    ![](assets/2023-04-11-14-12-24.png)

    - 이렇게 칼럼별로 다 설정..

    ![](assets/2023-04-11-14-13-23.png)

### 값타입 vs 불변객체

- 값타입은 단순하고 안전하게 다뤄야함. 그런데 값타입을 여러 엔티티에서 공유해버릴수 있음.

    ![](assets/2023-04-11-14-16-49.png)

    ![](assets/2023-04-11-14-20-02.png)

    - 이런 버그.. 하기 힘듬
    - 이럴때는 값타입 아니라 엔티티임.

- 그런데 위처럼 직접 지정한타입(만든 클래스)는 부작용을 피하기 힘듬. ( 참조값을 대입하는걸 막을 수 없음. )
- 그래서 이걸 막기 위해서 값타입은 불변객체로 설계해야함
  - 생성자로만 만들고 Setter는 안 만든다.

- 값타입간 비교
  - 인스턴스 달라도 안의 값이 같으면 같게 해야하니까 `equals` 를 잘 정의하자


### 값 타입 컬렉션

- 일대다 테이블 관계로 풀어야하는게 일반적인데.. 이걸 쉽게 해보자.

![](assets/2023-04-11-14-57-30.png)

  - 그런데 값타임은 그 값들이 고유하기때문에 복합 PK로 설정하자

  ![](assets/2023-04-11-15-04-59.png)

  - `@ElementCollection` / `@CollectionTable` / `@Column` 등 활용

  ![](assets/2023-04-11-15-07-46.png)

  ![](assets/2023-04-11-15-07-59.png)

  ![](assets/2023-04-11-15-08-11.png)

    - 음 그런데 키는 안걸렸네? 음...


  ![](assets/2023-04-11-15-14-44.png)

  ![](assets/2023-04-11-15-14-25.png)
    - 생성 한번에 잘됬네
    - 그래서 보면 값타입 컬렉션은 `CASCADE` + `고아객체 제거 기능`이 합쳐진걸로 볼수 잇음

    ![](assets/2023-04-11-15-17-49.png)

  - 조회를 한번 해보면

  ![](assets/2023-04-11-15-21-39.png)
